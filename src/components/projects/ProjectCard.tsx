...
const ProjectCard = ({
  project: { id, title, descriptions, images, url },
}: {
  project: IProject;
}) => {
  return (
    <div className="bg-background-gradient flex p-3 lg:p-4 rounded-2xl lg:rounded-3xl shadow-sm border border-dark-blue group">
      <div className="flex flex-col justify-between gap-3 w-full">
        <div className="bg-[#000F22]/40 rounded-2xl lg:rounded-3xl overflow-hidden pt-2 shadow-md">
          <div className="h-56 sm:h-64 md:h-80 flex justify-center items-center ">
            <img
              src={`${import.meta.env.VITE_API_URL}/storage/${images[0].image}`}
              alt={title}
              className="w-full h-full object-cover rounded-t-2xl lg:rounded-t-3xl transition-transform duration-300 group-hover:scale-105"
            />
          </div>
        </div>
        <div className="flex flex-col gap-2 justify-center items-center lg:justify-start lg:items-start">
          ...
          <p className="break-all text-center lg:text-start text-sm lg:text-base leading-relaxed text-muted max-w-md line-clamp-2">
            {descriptions}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row justify-between sm:justify-center sm:gap-x-4 gap-y-2 lg:justify-between items-center">
          <Button className="bg-btn-primary hover:bg-btn-primary-hover rounded-lg w-full sm:w-fit">
            <Link
              to={`/projects/${id}`}
              className="flex items-center justify-center sm:justify-start gap-x-3 text-white py-3 px-4  md:text-start"
            >
              تفاصيل أكثر
              <CgArrowTopRightO className="h-5 w-5" size={20} />
            </Link>
          </Button>
          <Button className="bg-background hover:bg-background/80 rounded-lg w-full sm:w-fit">
            <Link
              to={url}
              target="_blank"
              className="flex items-center justify-center sm:justify-start gap-x-3 text-white py-3 px-4  md:text-start"

...
import { useUpdateEmployee } from "../../../lib/react-query/employees";
...
const UpdateEmployeeButton = ({ employee }: { employee: IEmployee }) => {
  const [selected, setSelected] = useState<string>(`${employee.user_id}`);
  ...
  return (
    <>
      <Button
        ...
        className="bg-btn-primary hover:bg-btn-primary-hover w-full text-white rounded-full py-3"
      >
        تعديل
      </Button>

      {/* update Modal */}
      <Modal isOpen={isOpen} closeModal={closeModal} title="تعديل موظف">
